...
class NodeBallJourney(object):
    ...
    def __init__(self, name="NodeBallJourney"):
        ...
        self.lostEvent = rospy.Publisher('soccer/lostBall', String, queue_size=1)
        self.finishedEvent = rospy.Publisher('/soccer/balljourney/finished', Bool, queue_size=1)
                ...
                if self.correctHeading():
                    if self.ballMessage.ballDetected:
                        if self.avgDistance() > MIN_DISTANCE:
                            ...
                            print "Ende"
                            self.finishedEvent.publish(True)
                            time.sleep(1)
                            self.run = False
...
# CALLBACKS
    ...
    def unsubscribe(self):
        self.ballDetection.unregister()
        self.imuData.unregister()

    def subscribe(self):
        self.ballDetection = rospy.Subscriber("/soccer/balldetection/ballPosition", String, self.detectionCallBack, queue_size = 1)
        self.imuData = rospy.Subscriber("/mobile_base/sensors/imu_data", Imu, self.directionCallback, queue_size = 1)
